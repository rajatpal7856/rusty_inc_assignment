...
<script type="module">
	import { ui } from '<?php echo esc_url( $ui_js_url )?>';
 	import { subscribe, updateTree, updateSecretURL, updateNonce } from '<?php echo esc_url( $framework_js_url )?>';
 	const tree = <?php echo $tree_js ?>;
 	const url = <?php echo $secret_url ? "'" . $secret_url . "'" : 'null'; ?>;
 	const rustyIncNonce = '<?php echo wp_create_nonce( 'rusty_inc_tree' ) ?>';
 	subscribe( ( newTree, newURL, newExpanded ) => ui( tree, url, newExpanded, document.getElementById( 'ui' ) ) );
 	updateTree( tree );
 	updateSecretURL( url );
 	updateNonce( rustyIncNonce );
</script>
